...
自 0.5.0 起，插件会在 `pages.json` 的 `pages` 与每个 `subPackages` 条目的头部写入生成标记注释（如 `// GENERATED BY UNI-PAGES, PLATFORM: H5 || MP-WEIXIN`），用于记录上一次运行写入过的平台。

基于该标记，插件会自动收敛由它生成、但在当前平台扫描中已不存在的条目：

- **页面/子包条目**：带生成标记且本次扫描未命中的子包，会被剥离当前平台的条目；其他平台的 `#ifdef` 条目保留。若某个平台的所有页面都被剥离后子包收敛为空，整个子包条目会从 `pages.json` 中删除。
